...
test('id에 따라 그에 맞는 name과 price 반환', () => {
  ...
  const product_id = PRODUCTS_MOCK_DATA.content[0].id.toString();
  ...
  (useGetProductDetail as jest.Mock).mockImplementation(() => ({
    data: PRODUCTS_MOCK_DATA.content.find((product) => product.id.toString() === product_id),
  }));

  // When: GoodsDetailHeader 컴포넌트를 렌더링하면
  render(<GoodsDetailHeader product_id={product_id} />);
