...
const getProducts = async (params: GetProductsRequest, baseURL: string): Promise<GetProductsResponse> => {
  const instance = initInstance(baseURL);
  const res = await instance.get(PRODUCTS_PATHS.PRODUCTS(params));
  return res.data;
};
...
export const useGetProducts = ({
  ...
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => getProducts({ categoryId, page: pageParam, size, sort }, baseURL),
    initialPageParam: 0,
