...
Github Pages나 Cloudflare Pages와 같은 다른 정적 배포 서비스를 이용하면 된다. 대신 CI/CD setup이 필요할 수 있다.

- 질문 2. CSRF나 XSS 공격을 막는 방법은 무엇일까요?

csrf 공격은 브라우저 세션을 이용하여 사용자가 의도하지 않은 동작을 실행하도록 하는 공격 방식이다.
cookie, session를 이용한 인증 방식을 이용하는 애플리케이션의 경우 csrf 공격에 취약할 수 있다.

csrf 공격을 막기 위해서는 요청 헤더의 `Referer` 값이 origin(host)와 일치하는지 확인하거나, 
렌더링할 문서 내부에 csrf 토큰을 넣어 방어할 수 있다.

xss 공격은 cross site scripting의 약어로, 웹 애플리케이션에 악성 스크립트를 삽입하여
사용자의 브라우저에서 실행되도록 하는 공격 기법이다. 공격자는 xss공격을 통해 세션 탈취나 인증 토큰 탈취, 피싱 사이트로의 리디렉션 등이 가능하다.

우선 리액트를 사용할 경우, 사용자 입력이 html태그로 입력되더라도 자동으로 일반 텍스트로 변환해준다. 원리는 `<`와 `>`문자를 각각 `&lt;`, `^gt;`로 대체하는 것이다.
그렇더라도 클라이언트의 데이터는 언제든 변조가 가능하기 때문에, 백엔드 측에서 사용자 input에 악성 스크립트가 포함되어 있는지 검증하는 과정은 필수이다. 

클라이언트는 사용자의 input을 통해 만들어진 데이터를 바로 innerHTML로 넣는 등의 주입을 최대한 지양해야 한다.

- 질문 3. 브라우저 렌더링 원리에 대해 설명해주세요.

1. HTML문서와 CSS를 파싱하여 DOM, CSSOM 트리 생성
2. CSSOM과 DOM을 결합하여 렌더 트리 생성. 렌더 트리에는 각 DOM요소가 실제로 화면에 어떻게 그려질지 나타낸다.
3. Layout: 렌더 트리를 기반으로 각 요소의 정확한 위치를 계산하는 과정. font-size, width, height등의 크기와 display 속성 등이 영향을 미칠 수 있다.
4. Paint: 각 요소의 스타일, 위치 등을 고려하여 픽셀로 변환하는 단계이다.
5. Composite: paint된 레이어를 화면에 표시하는 단계이다.

각 요소의 위치에 변화가 생기면 Reflow (Layout + Paint 재실행), 요소의 스타일링에 변화가 생기면 Repaint(Paint만 재실행)가 발생한다.
여기서 Reflow는 비교적 무거운 작업이므로 줄이는 것이 좋다.
